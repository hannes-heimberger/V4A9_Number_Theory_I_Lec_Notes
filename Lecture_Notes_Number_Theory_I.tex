...
Let $\{ \vec{b}_1, \dots, \vec{b}_n \}$ be linearly independent vectors in $\R^n$.
The Gram--Schmidt orthogonalisation process produces vectors
$\{ \vec{b}_1', \dots, \vec{b}_n' \}$ defined recursively by
\[
\vec{b}_1' = \vec{b}_1,
\]
\[
\vec{b}_2' = \vec{b}_2 - \mu_{2,1}\vec{b}_1',
\]
\[
\vdots
\]
...
\vec{b}_n
-
\sum_{j=1}^{n-1}\mu_{n,j}\vec{b}_j',
\]
where
\[
\mu_{i,j}
=
\frac{\inp{\vec{b}_i}{\vec{b}_j'}}
     {\inp{\vec{b}_j'}{\vec{b}_j'}}
\qquad
...
\[
\inp{\vec{b}_i'}{\vec{b}_j'} = 0
\qquad \text{for } i \neq j.
\]
Recall that we can write
\[ \vec{b}_i = \vec{b}_i' + \underbrace{\sum_{j=1}^{i-1} \mu_{i,j} \vec{b}_j'}_{\text{Projection of } \vec{b}_i \text{ onto } \Span_{\R} \{\vec{b}_1', \dots, \vec{b}_{i-1}'\}} \]
and
\[ \Span_{\R} \{\vec{b}_1, \dots, \vec{b}_n\} = \Span_{\R} \{\vec{b}_1', \dots, \vec{b}_n'\}. \]

\para{Recall:}
We can write the vector $\vec{b}_i$ as:
\[ \vec{b}_i = \vec{b}_i' + \sum_{j=1}^{i-1} \mu_{i,j} \vec{b}_j' \]
where $\sum_{j=1}^{i-1} \mu_{i,j} \vec{b}_j'$ is the projection of $\vec{b}_i$ onto $\Span_{\mathbb{R}} \{ \vec{b}_1, \dots, \vec{b}_{i-1} \}$.
Note that $\Span_{\mathbb{R}} \{ \vec{b}_1, \dots, \vec{b}_n \} = \Span_{\mathbb{R}} \{ \vec{b}_1', \dots, \vec{b}_n' \}$.

\begin{definition}{Reduced Basis}{reduced_basis}
    A basis $\{ \vec{b}_1, \dots, \vec{b}_n \}$ of a lattice $\Lambda \subseteq \mathbb{R}^n$ is \textbf{reduced} if:
    \begin{enumerate}[label=\roman*)]
        \item $|\mu_{i,j}| \le \frac{1}{2}$ for $1 \le j < i \le n$.
        \item $\|\vec{b}_i' + \mu_{i,i-1} \vec{b}_{i-1}'\|_2^2 \ge \frac{3}{4} \|\vec{b}_{i-1}'\|_2^2$ for $2 \le i \le n$.
    \end{enumerate}
\end{definition}

\titremark{The constant $3/4$} The choice of the constant $\frac{3}{4}$ is not crucial; any real number in the interval $(\frac{1}{4}, 1)$ works. Condition ii) essentially says that the highlighted portion of $\vec{b}_i$ is longer (up to a constant factor) than $\vec{b}_{i-1}'$.

\begin{proposition}{Properties of Reduced Bases}{2.19}
    Let $\{ \vec{b}_1, \dots, \vec{b}_n \}$ be a reduced basis of $\Lambda$. Let $\{ \vec{b}_1', \dots, \vec{b}_n' \}$ be the Gram-Schmidt orthogonalization. Then:
    \begin{enumerate}
        \item $\|\vec{b}_l\|_2^2 \le 2^{i-1} \|\vec{b}_i'\|_2^2$ for $1 \le l \le i \le n$.
        \item $\det \Lambda \le \|\vec{b}_1\|_2 \cdots \|\vec{b}_n\|_2 \le 2^{\frac{n(n-1)}{4}} \det \Lambda$.
        \item $\|\vec{b}_1\|_2 \le 2^{\frac{n-1}{4}} (\det \Lambda)^{1/n}$.
    \end{enumerate}
\end{proposition}
...
\begin{pf}
    ...
    \[ \|\vec{b}_i'\|_2^2 + \mu_{i,i-1}^2 \|\vec{b}_{i-1}'\|_2^2 \ge \frac{3}{4} \|\vec{b}_{i-1}'\|_2^2 \]
    Since $|\mu_{i,i-1}| \le \frac{1}{2}$, we have $\mu_{i,i-1}^2 \le \frac{1}{4}$, which gives:
    \[ \|\vec{b}_i'\|_2^2 \ge \left( \frac{3}{4} - \frac{1}{4} \right) \|\vec{b}_{i-1}'\|_2^2 = \frac{1}{2} \|\vec{b}_{i-1}'\|_2^2 \]
    By induction, $2^{i-l} \|\vec{b}_i'\|_2^2 \ge \|\vec{b}_l'\|_2^2$ for $1 \le l \le i$ ($\star$).
    Now, relating $\|\vec{b}_l\|_2$ to $\|\vec{b}_l'\|_2$:
    \[ \|\vec{b}_l\|_2^2 = \|\vec{b}_l'\|_2^2 + \sum_{j=1}^{l-1} \mu_{l,j}^2 \|\vec{b}_j'\|_2^2 \le \|\vec{b}_l'\|_2^2 + \sum_{j=1}^{l-1} \frac{1}{4} \cdot 2^{l-j} \|\vec{b}_l'\|_2^2 \]
    Summing the geometric series yields $\|\vec{b}_l\|_2^2 \le 2^{l-1} \|\vec{b}_l'\|_2^2$. Combined with ($\star$), we get the result.

    \textbf{(2)} The lower bound is Hadamard's inequality. For the upper bound, we use $\|\vec{b}_i\|_2 \le 2^{\frac{i-1}{2}} \|\vec{b}_i'\|_2$ and the fact that $\det \Lambda = \prod \|\vec{b}_i'\|_2$.

    \textbf{(3)} Applying (1) with $l=1$ gives $\|\vec{b}_1\|_2^2 \le 2^{i-1} \|\vec{b}_i'\|_2^2$. Taking the product over $i=1, \dots, n$ leads to the bound.
\end{pf}

\begin{proposition}{Shortest Vector Approximation}{2.20}
    Let $\{ \vec{b}_1, \dots, \vec{b}_n \}$ be a reduced basis of $\Lambda$. For any $\vec{0} \neq \vec{x} \in \Lambda$:
    \[ \|\vec{b}_1\|_2^2 \le 2^{n-1} \|\vec{x}\|_2^2 \]
\end{proposition}

\titremark{Remark} This implies that $\vec{b}_1$ is within a factor of $2^{\frac{n-1}{2}}$ of the shortest vector. The LLL algorithm computes such a basis efficiently. In modern cryptography, $n$ often ranges from 256 to over 1024.

\begin{pf}
    Write $\vec{x} = \sum_{i=1}^n m_i \vec{b}_i$ with $m_i \in \mathbb{Z}$. Let $j_0$ be the largest index such that $m_{j_0} \neq 0$.
    Then $\vec{x} = \sum_{i=1}^{j_0} \alpha_i \vec{b}_i'$ where $\alpha_{j_0} = m_{j_0}$.
    Thus $\|\vec{x}\|_2^2 \ge \alpha_{j_0}^2 \|\vec{b}_{j_0}'\|_2^2 \ge \|\vec{b}_{j_0}'\|_2^2$.
    Using Prop \ref{prop:2.19}(1), we have $\|\vec{b}_{j_0}'\|_2^2 \ge 2^{-(j_0-1)} \|\vec{b}_1\|_2^2$, which completes the proof.
\end{pf}
...
\begin{proposition}{Generalization}{2.21}
    ...
    \[ \|\vec{b}_i\|_2^2 \le 2^{n-1} \max \{ \|\vec{x}_1\|_2^2, \dots, \|\vec{x}_\ell\|_2^2 \} \quad \text{for all } 1 \le i \le \ell. \]
\end{proposition}
...
\begin{definition}{Property (k)}{def:prop_k}
    Let $2 \le k \le n+1$. We say a basis $\{ \vec{b}_1, \dots, \vec{b}_n \}$ satisfies \define{Property (k)} if:
    \begin{enumerate}[label=\roman*)]
        \item $|\mu_{i,j}| \le \frac{1}{2}$ for all $1 \le j < i < k$.
        \item $||\vec{b}_i' + \mu_{i,i-1}\vec{b}_{i-1}'||_2^2 \ge \frac{3}{4} ||\vec{b}_{i-1}'||_2^2$ for all $2 \le i < k$.
    \end{enumerate}
\end{definition}
...
We start with a basis $B_0 = \{ \vec{b}_1, \dots, \vec{b}_n \}$ of $\Lambda$. In particular, it satisfies Property (2).
...
\titremark{Note on efficiency} The algorithm is efficient. If we start with a basis $\{ \vec{b}_1, \dots, \vec{b}_n \}$ with $||\vec{b}_i||_2 \le B$ ($1 \le i \le n$), then the number of arithmetical operations (addition, subtraction, multiplication, division) needed in the algorithm is $\mathcal{O}(n^4 \log B)$, and the algorithm runs in polynomial time in $B$.
...
If $(*)$ does not hold, let $r \in \mathbb{Z}$ be such that $|\mu_{k,k-1} - r| \le \frac{1}{2}$. We replace $\vec{b}_k$ with $\vec{b}_k - r\vec{b}_{k-1}$. This has the effect of changing $\mu_{k,k-1}$ to $\mu_{k,k-1} - r$. Since $|\mu_{k,k-1} - r| \le \frac{1}{2}$, the new basis satisfies $(*)$.

\begin{addcalc}
Extra explanation: If we write $\vec{c}_k = \vec{b}_k - r\vec{b}_{k-1}$, then its Gram-Schmidt vector is:
...
Note: $\vec{b}_i$, $\vec{b}_i'$ and $\mu_{i,j}$ are not affected for $1 \le i \le k-1$.
\end{addcalc}
...
Then we switch $\vec{b}_k$ and $\vec{b}_{k-1}$ while keeping the rest the same. Our new basis is $\vec{c}_1, \dots, \vec{c}_n$ with $\vec{c}_{k-1} = \vec{b}_k$, $\vec{c}_k = \vec{b}_{k-1}$, and $\vec{c}_i = \vec{b}_i$ otherwise.
This changes $\mu_{i,j}$ and $\vec{c}_i'$, but it does NOT affect $\vec{b}_i, \vec{b}_i', \mu_{i,j}$ for $1 \le i \le k-2$. The new basis still satisfies Property (k-1).

Now $\vec{c}_{k-1}'$ is the projection of $\vec{c}_{k-1} = \vec{b}_k$ onto $(\Span_{\mathbb{R}}\{\vec{b}_1', \dots, \vec{b}_{k-2}'\})^\perp$, i.e.,
...
$$ ||\vec{b}_k' + \mu_{k,k-1}\vec{b}_{k-1}'||_2^2 \ge \frac{3}{4} ||\vec{b}_{k-1}'||_2^2 \quad (***) $$
First note that we already have $|\mu_{k,k-1}| \le 1/2$ (from $(*)$). Let $l$ be the largest integer such that $|\mu_{k,l}| > 1/2$. (Recall $\mu_{i,j}$ is only defined for $1 \le j < i \le n$). Let $r \in \mathbb{Z}$ be such that $|\mu_{k,l} - r| \le 1/2$. We then replace $\vec{b}_k$ with $\vec{b}_k - r\vec{b}_l$. Thus $\mu_{k,l}$ becomes $\mu_{k,l} - r$.

This change affects other $\mu_{k,j}$ for $j < l$, but it does not affect $\vec{b}_i, \vec{b}_i', \mu_{i,j}$ for $i < k$. It also doesn't affect $(***)$, as $l < k-1$. We repeat this process until we obtain that the basis satisfies $|\mu_{k,j}| \le 1/2$ for all $1 \le j \le k-1$. The final basis satisfies Property (k+1).
...
It can be checked that in both Cases 0 and 2, $D_1, \dots, D_n$ (and hence $\Delta$) do not change. (It only involves modification of the form $\vec{b}_k \to \vec{b}_k - c\vec{b}_i$ for some $i < k$).

And, in Case 1 only $D_{k-1}$ changes:
$$ D_{k-1} = ||\vec{b}_1'||_2^2 \cdots ||\vec{b}_{k-1}'||_2^2 \to ||\vec{c}_1'||_2^2 \cdots ||\vec{c}_{k-1}'||_2^2 $$
$$ = ||\vec{b}_1'||_2^2 \cdots ||\vec{c}_{k-1}'||_2^2 < \frac{3}{4} ||\vec{b}_1'||_2^2 \cdots ||\vec{b}_{k-1}'||_2^2 = \frac{3}{4} D_{k-1, \text{old}} $$
...
\begin{lemma}{Lower Bound for $\Delta$}{lem:2.21}
    Let $\{ \vec{b}_1, \dots, \vec{b}_n \}$ be a basis of $\Lambda$. Then
    $$ \Delta(\vec{b}_1, \dots, \vec{b}_n) \ge \frac{m(\Lambda)^{n^2}}{4^{n^2} V_n^{2n}} $$

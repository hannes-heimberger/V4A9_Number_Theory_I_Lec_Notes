...
\begin{pf}
Write
\[ x = m_{1}b_{1} + \dots + m_{n}b_{n} \quad \text{w/ } m_{1}, \dots, m_{n} \in \mathbb{Z} \]
\[ = \alpha_{1}b_{1}^{\prime} + \dots + \alpha_{n}b_{n}^{\prime} \quad \text{w/ } \alpha_{1}, \dots, \alpha_{n} \in \mathbb{R}. \]
Let $j_{0}$ be the largest index s.t.
\[ m_{j_{0}} \neq 0. \]

Then
\[ \Span_{\mathbb{R}}\{b_{1}, \dots, b_{j_{0}}\} = \Span_{\mathbb{R}}\{b_{1}^{\prime}, \dots, b_{j_{0}}^{\prime}\} \]
and it follows from the defn of $b_{1}^{\prime}, \dots, b_{n}^{\prime}$ that $m_{j_{0}} = \alpha_{j_{0}}$. And, also
...
&\ge 2^{-(j_{0}-1)} \cdot \|b_{1}\|_{2}^{2} \quad \text{\raggedleft (by Prop 2.19 \textcircled{1})}
\end{align*}
\[ \Rightarrow \|b_{1}\|_{2} \le 2^{\frac{j_{0}-1}{2}} \|x\|_{2} \le 2^{\frac{n-1}{2}} \|x\|_{2}. \]
\end{pf}


% --- SEITE 11 ---
\para{A small generalisation of the previous proposition.}

\begin{proposition}{}{prop2.21}
Let $\{b_{1}, \dots, b_{n}\}$ be a reduced basis of a lattice $\Lambda$. Let $1 \le \ell \le n$ and $x_{1}, \dots, x_{\ell} \in \Lambda$ be LI/$\mathbb{R}$.
...
\begin{pf}
For each $x_{i}$, write
\[ x_{i} = m_{i,1}b_{1} + \dots + m_{i,n}b_{n} \quad \text{w/ } m_{i,j} \in \mathbb{Z} \]
\[ = \alpha_{i,1}b_{1}^{\prime} + \dots + \alpha_{i,n}b_{n}^{\prime} \quad \text{w/ } \alpha_{i,j} \in \mathbb{R}. \]
Let $j_{0}(i)$ be the largest $j$ s.t. $m_{i,j} \neq 0$.


% --- SEITE 12 ---
Then, by the defn of $\{b_{1}^{\prime}, \dots, b_{n}^{\prime}\}$, it follows that
\[ \alpha_{i, j_{0}(i)+1} = \dots = \alpha_{i,n} = 0 \quad \text{and} \]
\[ 0 \neq m_{i, j_{0}(i)} = \alpha_{i, j_{0}(i)}, \text{ as before.} \]

We have
\[ \|x_{i}\|_{2}^{2} \ge \alpha_{i, j_{0}(i)}^{2} \|b_{j_{0}(i)}^{\prime}\|_{2}^{2} \ge \|b_{j_{0}(i)}^{\prime}\|_{2}^{2} \quad \dots (1) \]

Without loss of gen. (relabel $x_{1}, \dots, x_{\ell}$, if necessary) we assume
...
\para{Claim:} We have $i \le j_{0}(i)$.

Suppose otherwise. Then $j_{0}(i) < i$.
\[ \Rightarrow \{x_{1}, \dots, x_{i}\} \subseteq \bigcup_{h=1}^{i} \Span_{\mathbb{R}}\{b_{1}, \dots, b_{j_{0}(h)}\} = \Span_{\mathbb{R}}\{b_{1}, \dots, b_{j_{0}(i)}\} \]
BUT This is LI/$\mathbb{R}$. $\to\leftarrow$

$\therefore i \le j_{0}(i)$.

Using this, (1) becomes
\begin{align*}
\|x_{i}\|_{2}^{2} \ge \|b_{j_{0}(i)}^{\prime}\|_{2}^{2} &\ge 2^{-(j_{0}(i)-1)} \|b_{i}\|_{2}^{2} \quad \text{\raggedleft (by Prop 2.19 \textcircled{1})} \\
\Rightarrow \|b_{i}\|_{2} &\le 2^{\frac{j_{0}(i)-1}{2}} \|x_{i}\|_{2} \le 2^{\frac{n-1}{2}} \|x_{i}\|_{2} \\
&\le 2^{\frac{n-1}{2}} \cdot \max \{ \|x_{1}\|_{2}, \dots, \|x_{\ell}\|_{2} \}
\end{align*}
And this holds for any $1 \le i \le \ell$. $\hfill \square$
\end{pf}
\newLecture{15th May 2026}
\section{The LLL Algorithm}

The \textbf{LLL (Lenstra-Lenstra-Lovász) algorithm} turns a basis into a reduced basis (and thus gives a "short" vector in $\Lambda$, recall Proposition 2.20).

\begin{definition}{Property (k)}{def:prop_k}
    Let $2 \le k \le n+1$. We say a basis $\{ b_1, \dots, b_n \}$ satisfies \define{Property (k)} if:
...
\titremark{Note} Property ($n+1$) means the basis is reduced. And Property (2) holds trivially for any basis.
...
\begin{addcalc}
Extra explanation: If we write $c_k = b_k - rb_{k-1}$, then its Gram-Schmidt vector is:
$$ c_k' = c_k - \sum_{j=1}^{k-1} \frac{\langle c_k, b_j' \rangle}{\langle b_j', b_j' \rangle} b_j' = c_k - \sum_{j=1}^{k-1} (\mu_{k,j} - r\mu_{k-1,j}) b_j' = b_k' $$
...
$$ \|b_k' + \mu_{k,k-1}b_{k-1}'\|_2^2 < \frac{3}{4} \|b_{k-1}'\|_2^2 \quad (**) $$
...
From $(**)$, we get $\|c_{k-1}'\|_2^2 < \frac{3}{4} \|b_{k-1}'\|_2^2$. So the length of the "new" $c_{k-1}'$ is shorter than the "old" $b_{k-1}'$ by a factor of $3/4$.
...
$$ \|b_k' + \mu_{k,k-1}b_{k-1}'\|_2^2 \ge \frac{3}{4} \|b_{k-1}'\|_2^2 \quad (***) $$
...
This change affects other $\mu_{k,j}$ for $j < l$, but it does not affect $b_i, b_i', \mu_{i,j}$ for $i < k$. It also doesn't affect $(***)$, as $l < k-1$. We repeat this process until we obtain that the basis satisfies $|\mu_{k,j}| \le 1/2$ for all $1 \le j \le k-1$. The final basis satisfies Property (k+1).
...
$$ D_{k-1} = \|b_1'\|_2^2 \cdots \|b_{k-1}'\|_2^2 \to \|c_1'\|_2^2 \cdots \|c_{k-1}'\|_2^2 $$
$$ = \|b_1'\|_2^2 \cdots \|c_{k-1}'\|_2^2 < \frac{3}{4} \|b_1'\|_2^2 \cdots \|b_{k-1}'\|_2^2 = \frac{3}{4} D_{k-1, \text{old}} $$
But all the other $D_i$ do not change $\Rightarrow \Delta_{\text{new}} < \frac{3}{4} \Delta_{\text{old}}$.

But $\Delta$ cannot get arbitrarily small:

\begin{lemma}{Lower Bound for $\Delta$}{lem:2.21}
...
We will need several auxilliary lemmata.
  ...
  \begin{pf}
    ...
    \[ \rank((z_i^j)_{\stackrel{1 \leq i \leq r}{j \in \mathfrak{N}}}) = r, \text{ where } \mathfrak{N} \subseteq \mathfrak{M} \text{ with } \#\mathfrak{N} = r.\]
    ...
    \begin{align*}
      ...
      \coloneq \quad & \det\begin{pmatrix}
        ...
        & z_r^k
      \end{pmatrix} \\
      = \quad & \sum_{j \in S(\mathfrak{N}) \setminus \{ k\}} \epsilon_j\det(A_j)x^{j_1}y^{j_2},
    \end{align*}
    ...
    \[ P \in \Span_{\R}(\mathfrak N \cap \{ x^{k_1}y^{k_2}\}) \subseteq \Span_{\R}(\mathfrak M).\]

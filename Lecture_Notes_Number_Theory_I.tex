...
\begin{equation*}
    \Lambda^*
    = \{x \in \R^n \mid \langle x,y\rangle \in \Z \text{ for all } y \in \Lambda\}.
\end{equation*}
...
\begin{equation*}
    \begin{split}
        \Lambda^*
        &= \{x \in \R^n \mid \inp{x}{y} \in \Z \text{ for all } y \in \Lambda\} \\
        &= \{x \in \R^n \mid \inp{x}{A z} \in \Z z \in \Z^n\} \\
        &= \{x \in \R^n \mid \inp{A^t x}{z} \in \Z z \in \Z^n\} \\
...
\begin{theorem}{}{}
    Let \(\Lambda\) be a lattice in~\(\R^n\) and let \(\lambda_1, \dots, \lambda_n\) be its successive minima w.\,r.\,t.\ \(B = \{x \in \R^n \mid \|x\|_2 \le 1\}\).
...
\begin{proof}
    ...
    Let \(v_1, \dots, v_n \in \Lambda\) be linearly independent over~\(\R\) and \(\|v_i\|_2 = \lambda_i\) for all \(1 \le i \le n\) (from Lem.~2.8).
...
\begin{corollary}
    ...
    Then, given any \(b \in \R^n\), there is \(x \in \Lambda\) such that
    \begin{equation*}
        \|b - x\|_2
        \le \frac{n c(n)}{2R}.
    \end{equation*}
...
\[ b_i = b_i' + \underbrace{\sum_{j=1}^{i-1} \mu_{i,j} b_j'}_{\text{Projection of } b_i \text{ onto } \Span_{\R} \{b_1', \dots, b_{i-1}'\}} \]
and
\[ \Span_{\R} \{b_1, \dots, b_n\} = \Span_{\R} \{b_1', \dots, b_n'\}. \]

\para{Recall:}
We can write the vector $b_i$ as:
\[ b_i = b_i' + \sum_{j=1}^{i-1} \mu_{i,j} b_j' \]
where $\sum_{j=1}^{i-1} \mu_{i,j} b_j'$ is the projection of $b_i$ onto $\Span_{\mathbb{R}} \{ b_1, \dots, b_{i-1} \}$.
Note that $\Span_{\mathbb{R}} \{ b_1, \dots, b_n \} = \Span_{\mathbb{R}} \{ b_1', \dots, b_n' \}$.

\begin{definition}{Reduced Basis}{reduced_basis}
    A basis $\{ b_1, \dots, b_n \}$ of a lattice $\Lambda \subseteq \mathbb{R}^n$ is \textbf{reduced} if:
...
\titremark{The constant $3/4$} The choice of the constant $\frac{3}{4}$ is not crucial; any real number in the interval $(\frac{1}{4}, 1)$ works. Condition ii) essentially says that the highlighted portion of $b_i$ is longer (up to a constant factor) than $b_{i-1}'$.
...
\begin{pf}
    \textbf{(1)} This follows from the definition. The statement is trivial for $i=1$.
    For $2 \le i \le n$, condition ii) implies:
    ...
    By induction, $2^{i-l} \|b_i'\|_2^2 \ge \|b_l'\|_2^2$ for $1 \le l \le i$ ($\star$).
    ...
    \textbf{(2)} The lower bound is Hadamard's inequality. For the upper bound, we use $\|b_i\|_2 \le 2^{\frac{i-1}{2}} \|b_i'\|_2$ and the fact that $\det \Lambda = \prod \|b_i'\|_2$.

    \textbf{(3)} Applying (1) with $l=1$ gives $\|b_1\|_2^2 \le 2^{i-1} \|b_i'\|_2^2$. Taking the product over $i=1, \dots, n$ leads to the bound.
\end{pf}
...
\titremark{Remark} This implies that $b_1$ is within a factor of $2^{\frac{n-1}{2}}$ of the shortest vector. The LLL algorithm computes such a basis efficiently. In modern cryptography, $n$ often ranges from 256 to over 1024.

\begin{pf}
    Write $x = \sum_{i=1}^n m_i b_i$ with $m_i \in \mathbb{Z}$. Let $j_0$ be the largest index such that $m_{j_0} \neq 0$.
    Then $x = \sum_{i=1}^{j_0} \alpha_i b_i'$ where $\alpha_{j_0} = m_{j_0}$.
    Thus $\|x\|_2^2 \ge \alpha_{j_0}^2 \|b_{j_0}'\|_2^2 \ge \|b_{j_0}'\|_2^2$.
    Using Prop \ref{prop:2.19}(1), we have $\|b_{j_0}'\|_2^2 \ge 2^{-(j_0-1)} \|b_1\|_2^2$, which completes the proof.
\end{pf}

\begin{proposition}{Generalization}{2.21}
    Let $\{ b_1, \dots, b_n \}$ be a reduced basis. If $x_1, \dots, x_\ell \in \Lambda$ are linearly independent, then:
    \[ \|b_i\|_2^2 \le 2^{n-1} \max \{ \|x_1\|_2^2, \dots, \|x_\ell\|_2^2 \} \quad \text{for all } 1 \le i \le \ell. \]
\end{proposition}
